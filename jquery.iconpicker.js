(function($) {
    ...
    $.fn.iconpicker = function(options) {
    	...
    	//Templates
    	this.templates = {
            filter: "<div class='row'><div class='col-xs-12 form-group'><div class='input-group'><span class='input-group-addon'>Filter: </span><input type='text' class='icon-filter form-control'></div></div></div>",
            sizes: "<div class='row'><div class='col-xs-12 form-group btn-group btn-group-justified'><a class='btn btn-default' href='#' data-size='sm'>SM</a><a class='btn btn-default' href='#' data-size='lg'>LG</a><a class='btn btn-default' href='#' data-size='2x'>2X</a><a class='btn btn-default' href='#' data-size='3x'>3X</a><a class='btn btn-default' href='#' data-size='4x'>4X</a><a class='btn btn-default' href='#' data-size='5x'>5X</a></div></div>",
        };
        ...
        //CSS href
        this.css = $(this.fa).attr("href");

        //Get and parse Font Awesome icons
        if (this.css !== undefined) {
            $.get(this.css, function(response) { self.parseCSS(response); })
                .fail(function() {
                    console.warn("Could not find Font-Awesome link");
                });
        } else {
            console.warn("Could not find Font-Awesome link");
        }

        //Parse CSS into array
        this.parseCSS = function(css) {
            var index = 1,
                icon,
                iconPattern = /\.fa-([^\.]*):before{content:["|']\\[^'|"]*["|']}/g;
            while (match = iconPattern.exec(css)) {
                icon = {
                    selector: "fa-" + match[index],
                    name: self.ucwords(match[index]),
                    filter: match[index],
                }
                this.icons[match[index]] = icon;
            }
            this.icons = this.sortObj(self.icons);
            if ($.isEmptyObject(this.icons)) {
                console.warn("No icons found in Font-Awesome CSS file");
            } else {
                this.createMenu();
            }
        }
